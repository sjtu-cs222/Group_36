...
class PrunningFineTuner_VGG16:
    ...
    def train(self, optimizer=None, epoches=10):
        ...
        for i in range(epoches):
            ...
            torch.save(self.model, "little_model_base")
    ...
    def prune(self):
        ...
        num_filters_to_prune_per_iteration = int(number_of_filters/3)-1
        ...
        for it_k in range(iterations):
            ...
            last_acc = self.train(optimizer, epoches=40)
            torch.save(model,"./pruned/littleNet_cifar10_prunned_%d_own_LittleNet" % it_k)
        ...
        torch.save(model, "littleNet_cifar10_prunned_own_LittleNet")
...
def get_args():
    ...
    parser.add_argument("--train_epoch", dest="train_epoch", action="store", type=int, default=40)
...
if __name__ == '__main__':
    ...
    if use_gpu:
        if args.train:
            ...
            if args.restore:
                model.load_state_dict(torch.load("little_model_base").state_dict())
        elif args.prune:
            model = torch.load("little_model_base").cuda()
    else:
        if args.train:
            ...
            model = torch.load("little_model_base")
    ...
    if args.train:
        ...
        torch.save(model, "little_model_base")
    elif args.prune:
        fine_tuner.prune()
    print("Done!\n")